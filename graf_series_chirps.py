...
try:
    # --- Cargar y preprocesar las series CHIRPS ---
    chirps = pd.read_csv(r'../../datos/chirps/csv/precip_tumaco_series_temporales.csv')
    chirps['timestamp'] = pd.to_datetime(chirps['Fecha'])  # Convertir a datetime
    chirps.drop(columns='Fecha', inplace=True)
    chirps.set_index('timestamp', inplace=True)
    chirps = chirps['2009':'2023']  # Filtrar por fechas
